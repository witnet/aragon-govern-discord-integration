...
if [[ "$VERSION" == "latest" ]]; then
    VERSION=$(curl --cacert /etc/ssl/certs/ca-certificates.crt -H "$AUTH" https://api.github.com/repos/witnet/aragon-govern-discord-integration/releases/latest | sed -En "s/.*\"tag_name\": \"([0-9|\.]+)\",/\1/p" | head -1)
fi

# The URL of the release file, and the name that will be used when downloaded
URL="https://api.github.com/repos/witnet/aragon-govern-discord-integration/tarball/$VERSION"
...
mv /tmp/witnet-aragon-govern-discord-integration-* /data/source &&
